...
TOOL_DECLARATIONS = [
...
},
    {
        "name": "save_memory",
        "description": (
            "Save a user fact to long-term memory ONLY when the user explicitly "
            "asks you to remember, save, store, or keep that information for future "
            "conversations, OR when the user clearly states a durable personal fact "
            "that is unrelated to the current task. "
            "NEVER call save_memory merely because the user is speaking Bisaya, "
            "English, or another language. "
            "NEVER call save_memory during woodworking calculations, equal-division "
            "calculations, board-feet calculations, cut lists, cabinet calculations, "
            "computer/system commands, volume, brightness, WiFi, microphone, camera, "
            "power commands, searches, or other task execution. "
            "If the user is asking a question or requesting an action, do NOT save "
            "memory unless they explicitly ask to remember something. "
            "Do NOT announce that you are saving memory. "
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        self._voice_gate_evaluated = False   # True once this turn's learned voice-gate rules have been checked
    ...
    def _check_voice_gate(self, text: str) -> dict:
        """Direct (non-tool-call) lookup of the memory_expert plugin's learned
        voice_gate rules. Called from the audio pipeline itself so enforcement
        does not depend on the model choosing to invoke the plugin. Fails open
        (allow=True) if the plugin isn't loaded or raises."""
        mod = sys.modules.get("plugins.memory_expert")
        fn = getattr(mod, "memory_v35_should_respond_to_voice", None) if mod else None
        if not callable(fn):
            return {"allow": True, "reason": "voice_gate_unavailable", "rule": None}
        try:
            result = fn(text)
            if isinstance(result, dict) and "allow" in result:
                return result
        except Exception as e:
            print(f"[JARVIS] âš  voice_gate check failed: {e}")
        return {"allow": True, "reason": "voice_gate_error", "rule": None}

    _THIRD_PERSON_MENTION_RE = re.compile(r"\b(si|ang|kang)\s+jarvis\b", re.IGNORECASE)

    def _looks_like_third_person_mention(self, text: str) -> bool:
        """Heuristic for Bisaya/English speech that talks ABOUT Jarvis rather than
        TO Jarvis, e.g. 'si Jarvis akong AI assistant' (describing him to someone
        else) vs 'Jarvis, buksi ang Chrome' (direct address, no si/ang marker
        before the name). Deliberately narrow: only fires on this specific
        subject-marker pattern so real commands that happen to include the name
        are never blocked. Independent of the LLM and of memory_expert's rules."""
        return bool(self._THIRD_PERSON_MENTION_RE.search(text or ""))
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        identity_ctx = (
            ...
            f"Do not say your own name in your responses unless the user "
            f"explicitly asks what your name is. Only engage when you are "
            f"clearly being addressed or commanded — do not respond just "
            f"because your name was mentioned in passing conversation that "
            f"is not directed at you.\n"
            f"{_addr}\n\n"
        )
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if name == "memory_auto_context":
            try:
                from plugins.memory_auto_context import run as _memory_auto_context_run

                _memory_auto_context_result = _memory_auto_context_run(
                    {
                        "action": args.get(
                            "action",
                            "get_context",
                ...
                result = _memory_auto_context_result or (
                    "No relevant memory context found."
                )

            except Exception as exc:
                print(
                    "[MemoryAutoContext] ERROR:",
                    repr(exc),
                )
                result = (
                    "Memory auto-context unavailable. "
        ...
        try:
            if name == "open_app":
                ...
                # Keep JARVIS internal mute state synchronized with
                # Windows microphone mute/unmute commands.
                mic_action = args.get("action") if isinstance(args, dict) else None

                if mic_action == "mic_mute":
                    self.ui.muted = True
                    print("[MIC SYNC] JARVIS UI muted = True")

                elif mic_action == "mic_unmute":
                    self.ui.muted = False
                    print("[MIC SYNC] JARVIS UI muted = False")
                ...
                if self._plugin_registry.has(name):
                    ...
                    # WOODWORKING AUTHORITATIVE VOICE
                    # The plugin performs the calculation locally.
                    # Gemini must NEVER recalculate woodworking numbers.
                    # For board-feet responses, use the exact local
                    # WOODWORKING VOICE RESULT sentence.
                    if name == "woodworking_skill" and result:
                        result = (
                            "[WOODWORKING AUTHORITATIVE RESULT]\n"
                            f"{result}\n"
                            "[WOODWORKING VOICE INSTRUCTION]\n"
                            "WOODWORKING IS LOCAL AND AUTHORITATIVE. "
                            "The numerical calculation above is FINAL. "
                            "NEVER recalculate, estimate, convert, reinterpret, "
                            "round, multiply, divide, or change any woodworking number. "
                            "For board-feet requests, speak ONLY the exact sentence "
                            "inside [WOODWORKING VOICE RESULT]. "
                            "Do not replace 'board feet' with BFT. "
                            "Do not change inches to another unit. "
                            "Do not change feet to inches. "
                            "Do not add or remove dimensions. "
                            "Do not create a different numerical answer. "
    ...
    async def _send_realtime(self):
        while True:
            ...
            try:
                ...
                print(f"[JARVIS] Realtime send stopped: {e}")
                return
        ...
        try:
            while True:
                async for response in self.session.receive():
                    ...
                    # Learned voice_gate check: runs once per turn, the moment JARVIS
                    # starts to answer (first audio chunk or output transcript), using
                    # whatever the user has said so far this turn. This is a direct,
                    # code-level enforcement of memory_expert's learned behavior rules â€”
                    # it does not depend on the model choosing to call the plugin.
                    _turn_starting = bool(
                        response.data
                        or (response.server_content
                            and response.server_content.output_transcription
                            and response.server_content.output_transcription.text)
                    )
                    if _turn_starting and not self._voice_gate_evaluated:
                        self._voice_gate_evaluated = True
                        _gate_text = " ".join(in_buf).strip()
                        if _gate_text:
                            _block_reason = None
                            if self._looks_like_third_person_mention(_gate_text):
                                _block_reason = "third_person_mention_heuristic"
                            else:
                                _gate = self._check_voice_gate(_gate_text)
                                if not _gate.get("allow", True):
                                    _block_reason = _gate.get("reason")
                            if _block_reason:
                                self._interrupted = True
                                print(f"[JARVIS] ðŸš« Voice-gate blocked response: "
                                      f"{_block_reason}")
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            self._voice_gate_evaluated = False
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        # STARTUP NEWS DISABLED — keep the Phase 1 greeting only.
        return
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    # Startup greeting ON — startup news remains disabled
                ...
                # drift) â€” drop them and reconnect with the plain config.
                if self._enhanced_live and (
                    ...
                    self.ui.write_log(
                        "SYS: Advanced audio features unavailable â€” reconnecting without them."
                    )
                    continue
